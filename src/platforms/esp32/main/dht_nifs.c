...
static term nif_dht_get_calibration(Context *ctx, int argc, term argv[])
{
    UNUSED(argc);
    term pin = argv[0];
    VALIDATE_VALUE(pin, term_is_integer);
    ...
    avm_int_t pin_val = term_to_int(pin);
    init_hanshake(pin_val);
    buf[0] = wait_while(pin_val, 1, MAX_WAIT_US);
    buf[1] = wait_while(pin_val, 0, HANDSHAKE_RECV_LOW_US);
    buf[2] = wait_while(pin_val, 1, HANDSHAKE_RECV_HIGH_US);
    for (int i = 0;  i < 80;  i += 2) {
        buf[3 + i] = wait_while(pin_val, 0, DATA_RECV_LOW_US);
        buf[3 + i + 1] = wait_while(pin_val, 1, DATA_RECV_HIGH_ONE_US);
    }
#ifdef ENABLE_TRACE
    TRACE("%d, %d, %d\n", buf[0], buf[1], buf[2]);
    for (int i = 0;  i < 80;  i += 2) {
        TRACE("%d, %d", buf[3 + i], buf[3 + i + 1]);
        if ((i + 2) % 16 == 0) {
            TRACE("\n");
        } else {
            TRACE(", ");
...
#endif
    return term_from_literal_binary(buf, 83, ctx);
}
...
static const struct Nif dht_get_calibration_nif =
{
    .base.type = NIFFunctionType,
    .nif_ptr = nif_dht_get_calibration
};
const struct Nif *dht_nifs_get_nif(const char *nifname)
{
    if (strcmp("dht:get_calibration/1", nifname) == 0) {
        TRACE("Resolved platform nif %s ...\n", nifname);
        return &dht_get_calibration_nif;
    }
