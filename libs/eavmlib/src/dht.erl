...
-export([
    ...
    measure/1, calibrate/1
]).
-export([get_calibration/1, read/1]). %% internal nif APIs
...
%% @hidden
-spec calibrate(DHT::dht()) -> binary().
calibrate(DHT) ->
    gen_server:call(DHT, calibrate).

%% @hidden
-spec get_calibration(Pin::pin()) -> binary().
get_calibration(_Pin) ->
    throw(nif_error).
...
handle_call(calibrate, _From, State) ->
    {LastMeasurmentTime, Response} = do_calibrate(State#state.pin, State#state.device, State#state.last_measurement),
    {reply, Response, State#state{last_measurement=LastMeasurmentTime}};
...
%% @private
do_calibrate(Pin, Device, LastMeasurement) ->
    maybe_sleep(Device, LastMeasurement),
